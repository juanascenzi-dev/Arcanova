...
import { buildWhatsAppUrl, buildEmailUrl, buildFacebookMessage, CONTACT_CONFIG } from '@/lib/leads';
...
export function ContactChannelSelector({ experienceId, experienceSlug, experienceName, onClose }: Props) {
  ...
  // Unified lead capture and redirect
  async function captureLead(channel: 'whatsapp' | 'email' | 'facebook', onSuccess?: () => void) {
    setSaving(true);
    setError(null);

    try {
      // Map people input to number
      let peopleNum: number | undefined = undefined;
      if (people) {
        if (people === '10+') {
          peopleNum = 10;
        } else if (people.includes('-')) {
          const [min] = people.split('-').map(Number);
          peopleNum = min;
        } else {
      ...
      setError('Error al guardar tu interés. Por favor, intenta de nuevo.');
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="border-t border-brand-navy/10 pt-5 mt-2">
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div>
          ...
          <input
            ...
            disabled={saving}
            className="w-full text-sm bg-brand-light border border-brand-navy/15 rounded-lg px-3 py-2 text-brand-navy focus:outline-none focus:border-brand-gold [color-scheme:light] disabled:opacity-60"
          />
        </div>
        <div>
          ...
          <select
            ...
            disabled={saving}
            className="w-full text-sm bg-brand-light border border-brand-navy/15 rounded-lg px-3 py-2 text-brand-navy focus:outline-none focus:border-brand-gold disabled:opacity-60"
          >
            <option value="">—</option>
      ...
      <div className="flex flex-col gap-2">
        <button
          ...
          disabled={saving}
          className="flex items-center justify-center gap-2.5 w-full py-3 rounded-xl bg-[#25D366] text-white font-bold text-sm hover:bg-[#1da851] transition-colors shadow-sm disabled:opacity-70"
        >
          {saving ? (
        ...
        <button
          onClick={handleEmail}
          disabled={saving}
          className="flex items-center justify-center gap-2.5 w-full py-3 rounded-xl bg-brand-navy text-white font-bold text-sm hover:bg-brand-ocean transition-colors shadow-sm disabled:opacity-70"
        >
          {saving ? (
            <Loader2 className="w-4 h-4 animate-spin" />
          ) : (
        ...
        <button
          ...
          disabled={saving}
          className="flex items-center justify-center gap-2.5 w-full py-3 rounded-xl border-2 border-[#1877F2] text-[#1877F2] font-bold text-sm hover:bg-[#1877F2] hover:text-white transition-colors disabled:opacity-70"
        >
          {saving ? (
            <Loader2 className="w-4 h-4 animate-spin" />
          ) : (
