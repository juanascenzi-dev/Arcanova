...
- **Experience catalog** — category filter pills + collapsible secondary filters (capacity bucket, price bucket, duration bucket) + sort-by dropdown (featured / price asc|desc / duration asc|desc / capacity asc|desc / A→Z). Cards show 3-column metadata: price (from USD), duration (e.g. "3–4h"), capacity (e.g. "Up to 8 people"). Modal stats bar also shows all three. No-results empty state with clear-filters CTA. `src/lib/service-display.ts` is the single source of truth for all parsing/display/filter/sort logic.
- **6 experience cards** with real Unsplash images, category filter tabs, and detail modals
- **Quote cart** — multi-experience cart persisted to `localStorage` (`austral_quote_cart`). `QuoteCartContext` (add/update/remove/clear per item with traveler breakdown, date range, booking rules, service pricing, optional override pricing, notes). `bookingRules` and `servicePricing` are now persisted inside each `QuoteItem` so recalculation on edit never loses them. `src/lib/quote.ts` has pure calculation and message-building utilities. `ContactChannelSelector` builds a full cart summary and opens WhatsApp/Email/Facebook with a consolidated inquiry message. Lead save is fire-and-forget (non-blocking). Items with no calculable price show "Price on request" / "Consultar precio" — never silent. Total section shows note when some services require manual pricing. `buildTravelerLine` is exported for UI reuse ("3 adults, 1 child" style).
- **Booking rules & service pricing** — DB columns `booking_rules` and `service_pricing` (JSONB) on `experiences` table. Each service has `chargeMode` (per_person / per_group_fixed / per_day / per_person_per_day / per_group_per_day), `minTravelers`, `maxTravelers`, `includedTravelers`, `chargeFullIncludedGroup`, and per-rate pricing fields. UI enforces max travelers in steppers and shows min-travelers warning. Subtotals are auto-computed from service rules; user can optionally override.
- **Facebook fix** — `m.me/rodrigo.fimiani` replaced with profile page `https://www.facebook.com/rodrigo.fimiani`. Clicking Facebook auto-copies the cart message to clipboard and shows a step screen with copyable text + a button to open the Facebook page.
- **Email fix** — uses temporary `<a>` element `.click()` instead of `window.location.href = mailto:` to work correctly in sandboxed iframe contexts.
- **Message format** — all cart messages use clean ASCII (no emoji). Emojis like 📅 💰 👥 render as ◆ replacement characters in some WhatsApp/email clients.
- **Admin editor** — "Pricing & Capacity" section added to ExperienceEditorModal: charge mode selector, min/max/base-size fields, chargeFullIncludedGroup toggle, and price fields (adult/child/senior/group/day) shown contextually by mode.
